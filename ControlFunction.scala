...
/** This bot builds a 'direction value map' that assigns an attractiveness score to
  ...
  *
  * The master bot uses the following state parameters:t
...
{    
    def forMaster(bot: Bot) {
        ...
        val dontSpawnGatherersUntil = bot.inputAsIntOrElse("dontSpawnGatherersUntil", -1)
        val lastDirection = bot.inputAsIntOrElse("lastDirection", 0)
        val generation = bot.inputAsIntOrElse("generation", -1) // impossible?
        ...
        if(dontFireAggressiveMissileUntil < bot.time && bot.energy > 100 && !nearestEnemyMaster.isEmpty) { // fire attack missile?
        ...
        if(dontFireDefensiveMissileUntil < bot.time && bot.energy > 100 && !nearestEnemySlave.isEmpty) { // fire defensive missile?
        ...
        if (dontSpawnGatherersUntil < bot.time && bot.energy > 200 && generation == 0) { // spawn new gatherer
            bot.spawn(direction.rotateClockwise90, "mood" -> "Gatherer", "createTime" -> bot.time)
            bot.set("dontSpawnGatherersUntil" -> (bot.time + 8))
        }
    }
    
    def keepGathering(bot: MiniBot) {
        val (directionValue, nearestEnemyMaster, nearestEnemySlave) = analyzeViewAsMaster(bot.view)
        ...
        // determine movement direction
        directionValue(lastDirection) += 20 // try to break ties by favoring the last direction
        val bestDirection45 = directionValue.zipWithIndex.maxBy(_._1)._2
        val direction = XY.fromDirection45(bestDirection45)
        bot.move(direction)
        bot.set("lastDirection" -> bestDirection45)
        ...
        val payoffs = for (i <- 2 to 10) yield (i, explosionPayoff(bot.view, bot.view.center, bot.energy, 5))
        val bestBet = payoffs.maxBy(_._2)
        
        if (bestBet._2 > bot.energy) bot.explode(bestBet._1)
        else if(dontFireAggressiveMissileUntil < bot.time && bot.energy > 100 && !nearestEnemyMaster.isEmpty) { // fire attack missile?
            nearestEnemyMaster match {
                case None =>            // no-on nearby
                case Some(relPos) =>    // a master is nearby
                    val unitDelta = relPos.signum
    ...
    def explosionPayoff(view: View, xy: XY, energy: Int, size: Int): Int = {
        val damageAtCenter = (energy / (size * size * 3.14)) * 200
        // just explosion area
        val trimmedView = view.trim(size, xy)
        val positions = for (i <- 0 to size * size) yield trimmedView.relPosFromIndex(i)
        positions.filter(pos => "msBb" contains trimmedView.cellAtRelPos(pos))
                 .map(pos => pos.distanceTo(trimmedView.center))
                 .map(dist => (max((1 - (dist / size)) * damageAtCenter, 100)))
                 .sum.toInt - energy
    }
    ...
    def avoidObstacles(bot: MiniBot, direction: XY): XY = {
        def isDeadly(dir: XY): Boolean = "Wmsb" contains bot.view.cellAtRelPos(dir)
        def isBad(dir: XY): Boolean = "Wmsbp" contains bot.view.cellAtRelPos(dir)
        
        def pickClosest(all: List[XY], dir: XY): XY = all.sortBy(d => d.distanceTo(dir)).head
        
        val allDirections = List(XY.Right, XY.RightUp, XY.Up, XY.UpLeft, XY.Left, XY.LeftDown, XY.Down, XY.DownRight)
        
        lazy val survivable = allDirections.filterNot(isDeadly _)
        lazy val good = allDirections.filterNot(isBad _)
        
        if (survivable.isEmpty) XY.Zero
        else if (good.isEmpty) pickClosest(survivable, direction)
        else pickClosest(good, direction)
    }
    
    def goHome(bot: MiniBot) {
        bot.move(avoidObstacles(bot, bot.inputAsXYOrElse("master", XY(0, 0))))
    }
    
    def reactAsGatherer(bot: MiniBot) {
        if (bot.energy < 1000 && bot.inputAsIntOrElse("createTime", 0) > bot.time - 120) forMaster(bot)
        else goHome(bot)
    }
...
case class View(cells: String) {
    ...
    def trim(r: Int, center: XY): View = {
        new View(cells.view.zipWithIndex
                    .filter(p => relPosFromIndex(p._2).distanceTo(center) <= r)
                    .map(p => p._1).mkString("")
                    )
    }
}
